...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run DB-OCSVM inference pipeline.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="./simulation_test_set.csv",
    ...
    parser.add_argument(
        "--dbocsvm",
        type=str,
        default="./dbocsvm_cidds_001.joblib",
        help="Path to DB-OCSVM joblib model",
    )
    ...
    # test_set_path = "./testing_dataset/simulation_test_set.csv"
    # autoencoder_onnx_path = "saved_models/onnx/autoencoder_encoder_cidds_001.onnx"
    # dbocsvm_model_path = "saved_models/joblib/dbocsvm_cidds_001.joblib"
    # debug = False
    ...
    test_set_features = test_df.shape[1] - 2  # Exclude the target columns
    attack_classes = test_df["attack_categorical"].value_counts()
    ...
    X_test_encoded_count = X_test_encoded.shape[0]
    X_test_encoded_features = X_test_encoded.shape[1]
    if debug:
        print(
            f"Latent representation shape: {X_test_encoded.shape}"
        )  # Should be (100, 9)
        print("")

    # Load the DB-OCSVM model
    ...
    if debug:
        print("Classification Report DB-OCSVM:")
    ...
    results = {
        "test_set": {
            ...
            "class_distribution": {
                "benign": attack_classes_dict.get("benign", 0),
                "dos": attack_classes_dict.get("dos", 0),
                "portScan": attack_classes_dict.get("portScan", 0),
                "bruteForce": attack_classes_dict.get("bruteForce", 0),
                "pingScan": attack_classes_dict.get("pingScan", 0),
            },
            "latent_shape": {
                "count": X_test_encoded_count,
                "features": X_test_encoded_features,
            },
        },
