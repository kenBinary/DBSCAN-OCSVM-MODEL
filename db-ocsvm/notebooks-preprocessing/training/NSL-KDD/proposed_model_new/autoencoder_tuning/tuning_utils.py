...
#     best_loss = float("inf")  # Changed best_val_loss to best_loss
...
#                 if verbose:
#                     print(f"✅ Model saved with val_loss: {best_loss:.10f}")
...
#                 if verbose:
#                     print(f"✅ Model saved with train_loss: {best_loss:.10f}")
...
#                         print(f"Early stopping triggered after {epoch+1} epochs")
#                     break
#         model.train() # ensure model is in train mode after each epoch
...
# this has early stopping and saved the training plots
def train_autoencoderV3(
    model,
    train_loader,
    optimizer,
    ...
    epochs=10,
    best_model_path="./best_autoencoder_tuning.pth",
    best_model_train_plot="./best_autoencoder_training_plot.png",  # New parameter for plot path
    verbose=False,
    patience=5,  # Add patience parameter for early stopping
):
    model.to(device)
    ...
    plt.savefig(best_model_train_plot)
    if verbose:
        print(f"✅ Training plot saved to {best_model_train_plot}")
    plt.close()
